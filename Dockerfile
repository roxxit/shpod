...
RUN apk add bash bash-completion curl git jq libintl ncurses openssl tmux vim apache2-utils openssh-client docker file sshpass zsh cntlm python3 python3-dev musl-dev libffi-dev libressl-dev openjdk11 gcc make ca-certificates socat terraform go
...
COPY --from=gh-cli /go/cli/bin/gh /usr/local/bin/gh
WORKDIR /root
